...
public class BoardControllerTest {
	...
	public void testGetBoard() throws Exception{
		// given : 대전제 - service.getBoard(1)이라는 요청을 하게 되면
		// 그러면 응답으로 boardVo 객체가 돌아올 것이다.
		...
		this.mockMvc.perform(get("/getBoard").param("bno", "1"))	
		
		// then
		.andExpect(status().isOk())		// 응답 상태가 200인지 확인한다.
		.andExpect(view().name("boardDetail"))	// boardDetail.jsp로 이동하는지 확인한다.
	...
	public void testGetBoardList() throws Exception{
		...
		Mockito.when(service.getBoardList())
				.thenReturn(boardList);
		
		// when
		// get방식으로 "/getBoardList"라는 요청을 한다.
		this.mockMvc.perform(get("/getBoardList"))	
		
		// then
		.andExpect(status().isOk())		// 응답 상태가 200인지 확인한다.
		.andExpect(view().name("boardList"))	// boardDetail.jsp로 이동하는지 확인한다.
	...
	public void testInsertBoard() throws Exception{
		...
		Mockito.when(service.insertBoard(Mockito.any(BoardVo.class))).thenReturn(1);
		...
		mockMvc.perform(post("/insertBoard")
				...
				.param("content", boardVo.getContent())
	...
	public void testUpdateBoard() throws Exception{
		...
		mockMvc.perform(post("/updateBoard")
				.contentType(MediaType.APPLICATION_FORM_URLENCODED)
				.param("bno", String.valueOf(boardVo.getBno()))
				.param("title", boardVo.getTitle())
				.param("content", boardVo.getContent())
				.param("memberId", boardVo.getMemberId()))
		// then
	...
	public void testDeleteBoard() throws Exception{
		...
		// 2. 테스트를 진행한다.
		// post() 메소드 : /insertBoard라는 요청을 보낸다.
		mockMvc.perform(post("/deleteBoard")
				.contentType(MediaType.APPLICATION_FORM_URLENCODED)
